...
def update_station_data(stations_data: Dict[str, Dict[str, float]], 
                       station: str, temperature: float) -> None:
    ...
    if station not in stations_data:
        ...
        stations_data[station]['high'] = max(stations_data[station]['high'], temperature)
        stations_data[station]['low'] = min(stations_data[station]['low'], temperature)


def generate_snapshot_output(stations_data: Dict[str, Dict[str, float]], 
                           latest_timestamp: int) -> dict[str, Any]:
